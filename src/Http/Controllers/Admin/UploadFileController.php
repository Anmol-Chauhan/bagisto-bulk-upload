...
class UploadFileController extends Controller
{
    ...
    {
        if (
            ! empty(request()->download_sample)
            && file_exists(public_path("vendor/webkul/admin/assets/sample-files/".request()->download_sample))
        ) {
    ...
    public function storeProductsFile()
    {
        ...
        /**
         * @var string[] $validImageExtensions
         */
    ...
    public function getProductImporter()
    {
        ...
        return [
            'dataFlowProfiles' => $productImporter,
        ];
    }
    ...
    public function deleteProductFile()
    {
        try {
            ...
            return [
                'importer_product_file' => $dataFlowProfile->import_product,
            ];
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            // Handle the case where the bulk_product_importer_id does not exist
            return response()->json([
                'error' => 'Data Flow Profile not found'
            ], 404);
        }
    }
    ...
    public function readCSVData()
    {
        ...
        $countCSV = $countConfig ? $countConfig : count($csvData);
        ...
        if (! empty($productFileRecord->image_path)) {
