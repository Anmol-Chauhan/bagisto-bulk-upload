...
class ProductRepository extends BaseRepository
{
    ...
    public function createProduct($imageZipName, $dataFlowProfileRecord, $csvData, $key)
    {
        try {
            ...
            if ($csvData['type'] == 'booking') {
                return response()->json([
                    'error' => [trans('bulkupload::app.admin.bulk-upload.messages.product-not-supported')],
                ]);
            }
            ...
            if ($csvData['type'] == 'configurable') {
                ...
                if (empty($superAttributes['super_attributes'])) {
                    return [
    ...
    {
        if (
            is_null($csvData['categories_slug'])
            || empty($csvData['categories_slug'])
        ) {
    ...
    {
        if (! empty($csvData['customer_group_prices'])) {
    ...
    public function addLinksAndSamples($csvData, $dataFlowProfileRecord, $product)
    {
        ...
        for ($index = 0; $index < $dataLength; $index++) {
            ...
            if (trim($linkData['link_types'][$index]) == "url") {
                ...
                trim($linkData['link_types'][$index]) == "file"
                && isset($fileLink)
            ) {
            ...
            if (trim($linkData['link_sample_types'][$index]) == "url") {
                ...
                trim($linkData['link_sample_types'][$index]) == "file"
