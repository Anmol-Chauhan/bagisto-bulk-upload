    ...
    @pushOnce('scripts')
        ...
        <script type="text/x-template" id="run-profile-form-template">
            <div class="run-profile">
                ...
                <div>
                    <p v-if="isProductUploaded" class="text-[20px] text-gray-800 dark:text-white font-bold">
                        ...
                        <span
                            ...
                            @click="forgetProductUploadedSessionDetails('uploadedProduct')"
                        >
                        </span>
                    </p>
                    <br>
                    ...
                    <ul class="overflow-y-auto whitespace-nowrap p-4" style="height: 150px;">
                        <li v-for="(item, index) in uploadedProductList" :key="index" class="dark:text-white">
                            @lang('bulkupload::app.admin.bulk-upload.upload-files.uploaded-product') :-
                            Product id: @{{ item.id }}
                ...
                <div>
                    <p v-if="isProductError" class="text-[20px] text-gray-800 dark:text-white font-bold">
                        @lang('bulkupload::app.admin.bulk-upload.run-profile.error-in-product')
                        <span
                            class="icon-cancel-1 text-[30px] cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-950 hover:rounded-[6px]"
                            @click="forgetProductUploadedSessionDetails('notUploadedProduct')"
                        >
                        </span>
                    </p>
                    <br>
                    ...
                    <ul class="overflow-y-auto whitespace-nowrap p-4" style="height: 150px;">
                        <li v-for="(item, index) in  notUploadedProductList" :key="index" class="dark:text-white">
                            @lang('bulkupload::app.admin.bulk-upload.upload-files.not-uploaded-product') :- @{{ item.error }}
                        </li>
                    </ul>
                </div>
                <br>
                ...
                <p v-if="errorCsvFile.length" class="text-[20px] text-gray-800 dark:text-white font-bold">
        ...
        <script type="module">
            ...
            app.component('run-profile-form', {
                ...
                methods: {
                    ...
                    forgetProductUploadedSessionDetails(sessionName) {
                        const uri = "{{ route('admin.bulk-upload.upload-file.run-profile.remove-details') }}"
                        var self = this;
